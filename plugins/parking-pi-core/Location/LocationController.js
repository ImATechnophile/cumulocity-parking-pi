(function () {
  ...
  function locationcontroller(
      ...
      geocoder.geocode({'location': latlng}, function(results, status) {
          if (status === 'OK') {
            if (results[0]) {
              ...
              console.log("location123",details.location);

              console.log(results[0].formatted_address);
              popupfunc(results[0].formatted_address);
            } else {
    ...
    var greenIcon = L.icon({
      iconUrl: 'parking_pi_parking-pi-core/Images/parking_marker_red.png',
    ...
    var marker = L.marker([pos.lat, pos.lng],{icon: greenIcon}).addTo(mymap).bindPopup(address)
    .openPopup();
